...
with open('/home/ubuntu/workspace/datasets/SportsQA/meta-data/test_all.json', 'r') as f:
    jf = json.load(f)

part = 1
jf = jf[5721:len(jf)//3*(part+1)]
...
for data in tqdm(jf):
    ...
    for k, v in number_dict.items():
        if k in text_outputs:
            text_outputs = text_outputs.replace(k, v)
    answer = data['answer'].lower()
    print(text_outputs, '///', answer, f'### part {part}')
