...
def load_pretrained_model_with_deepspeed(tokenizer, model, image_processor, max_length, device_map):  
    # Initialize DeepSpeed on the model
    ds_inference_config = {
        "mp_size": 1,  # Model parallelism size (can be set as needed)
        "dtype": torch.float16,  # Use fp16 precision for faster inference
        "replace_with_kernel_inject": True,  # Enables kernel injection for optimized transformer layers
        "zero": {
            "stage": 3,  # Modify stage as needed for memory vs. speed trade-offs
            "cpu_offload": True
        },
        "use_triton": True
    }
    model = deepspeed.init_inference(
        model, config=ds_inference_config
    )
...
model.eval()
tokenizer, model, image_processor, max_length = load_pretrained_model_with_deepspeed(tokenizer, model, image_processor, max_length, device_map)
